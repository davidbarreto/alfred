...
class TestGetNotes:
    def test_returns_list(self, client):
        response = client.get("/organizer/notes/", headers=AUTH)
    ...
    def test_requires_auth(self, client):
        response = client.get("/organizer/notes/")
        assert response.status_code == 403

    def test_wrong_token_rejected(self, client):
        response = client.get("/organizer/notes/", headers={"Authorization": "Bearer bad-token"})
        assert response.status_code == 401

    def test_tags_filter_passed_to_service(self, client, mock_service):
        client.get("/organizer/notes/?tags=work&tags=ideas", headers=AUTH)
...
class TestGetNote:
    def test_found_returns_200(self, client):
        response = client.get("/organizer/notes/1", headers=AUTH)
        assert response.status_code == 200
        assert response.json()["id"] == 1

    def test_not_found_returns_404(self, client, mock_service):
        mock_service.get_note.return_value = None
        response = client.get("/organizer/notes/999", headers=AUTH)
        assert response.status_code == 404
        assert response.json()["detail"] == "Note not found"

    def test_requires_auth(self, client):
        response = client.get("/organizer/notes/1")
        assert response.status_code == 403
...
class TestCreateNote:
    ...
    def test_requires_auth(self, client):
        response = client.post("/organizer/notes/", json={"title": "T"})
        assert response.status_code == 403
...
class TestUpdateNote:
    def test_updates_and_returns_200(self, client):
        ...
        response = client.patch("/organizer/notes/1", json=payload, headers=AUTH)
        assert response.status_code == 200
        assert response.json()["title"] == "Updated Note"

    def test_not_found_returns_404(self, client, mock_service):
        mock_service.update_note.return_value = None
        response = client.patch("/organizer/notes/999", json={"title": "X"}, headers=AUTH)
        assert response.status_code == 404

    def test_requires_auth(self, client):
        response = client.patch("/organizer/notes/1", json={"title": "X"})
        assert response.status_code == 403

    def test_invalid_note_id_type(self, client):
        response = client.patch("/organizer/notes/not-an-int", json={"title": "X"}, headers=AUTH)
        assert response.status_code == 422


class TestDeleteNote:
    def test_deletes_returns_204(self, client):
        response = client.delete("/organizer/notes/1", headers=AUTH)
        assert response.status_code == 204

    def test_requires_auth(self, client):
        response = client.delete("/organizer/notes/1")
        assert response.status_code == 403
