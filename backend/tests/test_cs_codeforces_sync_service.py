...
class TestRefreshMetrics:
    ...
    async def test_updates_problems_with_solved_count(self, service, mock_client):
        service._platforms.get_platform_by_code.return_value = _platform_orm()
        mock_client.get_problemset_statistics.return_value = {"100A": 4200, "200B": 900}
    ...
    async def test_skips_problems_missing_from_bulk_stats(self, service, mock_client):
        service._platforms.get_platform_by_code.return_value = _platform_orm()
        mock_client.get_problemset_statistics.return_value = {}
        service._problems.get_problems_by_platform.return_value = [_problem_orm(1, "100A")]

        updated = await service.refresh_metrics()

        assert updated == 0
        service._problems.update_problem.assert_not_called()
