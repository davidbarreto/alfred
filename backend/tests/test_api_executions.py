...
class TestListExecutions:
    def test_returns_empty_list(self, client):
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[])):
            response = client.get("/watcher/executions/", headers=AUTH)
    ...
    def test_returns_execution_list(self, client):
        ...
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=executions)):
            response = client.get("/watcher/executions/", headers=AUTH)
        assert response.status_code == 200
        assert len(response.json()) == 3

    def test_requires_auth(self, client):
        response = client.get("/watcher/executions/")
        assert response.status_code == 403

    def test_filter_by_config_id(self, client):
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[])) as mock_get:
            client.get("/watcher/executions/?config_id=5", headers=AUTH)
            _, kwargs = mock_get.call_args
            assert kwargs["filters"].config_id == 5

    def test_filter_by_status(self, client):
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[])) as mock_get:
            client.get("/watcher/executions/?status=found", headers=AUTH)
            _, kwargs = mock_get.call_args
            assert kwargs["filters"].status == "found"

    def test_filter_by_before_date(self, client):
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[])) as mock_get:
            client.get("/watcher/executions/?before_date=2026-06-01T00:00:00", headers=AUTH)
            _, kwargs = mock_get.call_args
            assert kwargs["filters"].before_date is not None

    def test_filter_by_after_date(self, client):
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[])) as mock_get:
            client.get("/watcher/executions/?after_date=2026-01-01T00:00:00", headers=AUTH)
            _, kwargs = mock_get.call_args
            assert kwargs["filters"].after_date is not None

    def test_filter_by_result(self, client):
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[])) as mock_get:
            client.get("/watcher/executions/?result=some+text", headers=AUTH)
            _, kwargs = mock_get.call_args
            assert kwargs["filters"].result == "some text"

    def test_default_pagination(self, client):
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[])) as mock_get:
            client.get("/watcher/executions/", headers=AUTH)
            _, kwargs = mock_get.call_args
    ...
    def test_execution_response_shape(self, client):
        ...
        with patch("app.api.routes.watcher.executions.get_all_executions", new=AsyncMock(return_value=[execution])):
            response = client.get("/watcher/executions/", headers=AUTH)
