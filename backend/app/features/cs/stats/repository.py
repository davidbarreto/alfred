...
class StatsRepository:
    ...
    async def get_daily_activity(self, days: int | None) -> list[tuple[datetime.date, int, int]]:
        """days=None means unbounded ("always")."""
        ...
        if days is not None:
            ...
            query = query.where(Submission.submitted_at >= since)
        result = await self._session.execute(query)
