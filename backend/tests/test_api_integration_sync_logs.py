...
class TestListSyncLogs:
    def test_returns_list(self, client):
        log = _sync_log_read()
        with patch(
            "app.api.routes.integrations.provider_calls.get_sync_logs",
            new=AsyncMock(return_value=[log]),
        ):
            response = client.get("/integration/provider-calls/", headers=AUTH)
    ...
    def test_requires_auth(self, client):
        assert client.get("/integration/provider-calls/").status_code == 403

    def test_wrong_token_rejected(self, client):
        assert client.get("/integration/provider-calls/", headers={"Authorization": "Bearer bad"}).status_code == 401
    ...
    def test_filters_forwarded(self, client):
        ...
        with patch("app.api.routes.integrations.provider_calls.get_sync_logs", new=mock_get):
            client.get("/integration/provider-calls/?provider=notion&entity_type=task&status=ok", headers=AUTH)
    ...
    def test_q_filter_forwarded(self, client):
        mock_get = AsyncMock(return_value=[])
        with patch("app.api.routes.integrations.provider_calls.get_sync_logs", new=mock_get):
            client.get("/integration/provider-calls/?q=timeout", headers=AUTH)
        _, kwargs = mock_get.call_args
    ...
    def test_operation_filter_forwarded(self, client):
        mock_get = AsyncMock(return_value=[])
        with patch("app.api.routes.integrations.provider_calls.get_sync_logs", new=mock_get):
            client.get("/integration/provider-calls/?operation=create", headers=AUTH)
        _, kwargs = mock_get.call_args
    ...
    def test_after_filter_forwarded(self, client):
        mock_get = AsyncMock(return_value=[])
        with patch("app.api.routes.integrations.provider_calls.get_sync_logs", new=mock_get):
            client.get("/integration/provider-calls/?after=2026-06-01T00:00:00Z", headers=AUTH)
        _, kwargs = mock_get.call_args
    ...
    def test_before_filter_forwarded(self, client):
        mock_get = AsyncMock(return_value=[])
        with patch("app.api.routes.integrations.provider_calls.get_sync_logs", new=mock_get):
            client.get("/integration/provider-calls/?before=2026-06-12T00:00:00Z", headers=AUTH)
        _, kwargs = mock_get.call_args
...
class TestGetSyncLog:
    def test_returns_log(self, client):
        ...
        with patch(
            "app.api.routes.integrations.provider_calls.get_sync_log",
            new=AsyncMock(return_value=log),
        ):
            response = client.get("/integration/provider-calls/1", headers=AUTH)
        assert response.status_code == 200
    ...
    def test_not_found_returns_404(self, client):
        with patch(
            ...
            response = client.get("/integration/provider-calls/99", headers=AUTH)
